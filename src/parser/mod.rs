    ...
    I: ValueInput<'tokens, Token = Token, Span = SimpleSpan> + 'tokens,
{
    recursive(|expr| {
        ...
        let atom = choice((
            ...
            ident.clone()
                .then(expr.clone().separated_by(just(Token::Comma)).allow_trailing().collect()
        ...
        let unary = just(Token::Not).to(UnaryOp::Not)
            ...
            .repeated()
            .foldr(atom, |op, right| Expr::Unary { op, expr: Box::new(right) });

        let op = |t| just(t).ignored();
        let product = unary.clone().foldl(
            choice((op(Token::Star).to(BinaryOp::Multiply), op(Token::SlashSlash).to(BinaryOp::FloorDivide), op(Token::Percent).to(BinaryOp::Modulo),))
                .then(unary).repeated(),
            |left, (op, right)| Expr::Binary { op, left: Box::new(left), right: Box::new(right) },
        );
        let sum = product.clone().foldl(
            choice((op(Token::Plus).to(BinaryOp::Add), op(Token::Minus).to(BinaryOp::Subtract),))
                .then(product).repeated(),
            |left, (op, right)| Expr::Binary { op, left: Box::new(left), right: Box::new(right) },
        );
        let comparison = sum.clone().foldl(
            choice((op(Token::Less).to(BinaryOp::Less), op(Token::LessEqual).to(BinaryOp::LessEqual), op(Token::Greater).to(BinaryOp::Greater), op(Token::GreaterEqual).to(BinaryOp::GreaterEqual), op(Token::EqualEqual).to(BinaryOp::Equal), op(Token::NotEqual).to(BinaryOp::NotEqual),))
                .then(sum).repeated(),
            |left, (op, right)| Expr::Binary { op, left: Box::new(left), right: Box::new(right) },
        );
    ...
    I: ValueInput<'tokens, Token = Token, Span = SimpleSpan> + 'tokens,
{
    ...
    recursive(|stmt| {
        ...
        let if_stmt = just(Token::If)
            ...
            .map(|(((condition, then_block), elif_blocks), else_block)| Stmt::If {
                condition,
                then_block,
                elif_blocks,
                else_block,
        ...
        choice((def_stmt, if_stmt, while_stmt, return_stmt, assign_stmt, expr_stmt))
...
pub fn program_parser<'tokens, I>() -> impl Parser<'tokens, I, Vec<Stmt>, extra::Err<RichError<'tokens>>>
where
    I: ValueInput<'tokens, Token = Token, Span = SimpleSpan> + 'tokens,
{
