...
impl Env {
    ...
    pub fn define(&mut self, name: String, val: Value, span: Span) -> Result<(), RuntimeError> {
        ...
        Err(RuntimeError {
            message: format!("failed to define '{}': no frame", name),
            span,
        })
    }
...
impl<IO: RuntimeIo> Interpreter<IO> {
        ...
        match &stmt.0 {
            ...
            Stmt::While { condition, body } => {
                loop {
                    ...
                    if !to_bool(&c, condition.1.clone())? {
                        break;
                    }
            ...
            Stmt::Def { name, params, body } => {
                ...
                self.env
                    .define(name.clone(), Value::Function(func), stmt.1.clone())?;
                Ok(None)
            }
        }
    }
            ...
            Expr::Binary { op, left, right } => {
                ...
                match op {
                    ...
                    B::FloorDivide => {
                        ...
                        let b = to_int(&rv, right.1.clone())?;
                        if b == 0 {
                            return Err(RuntimeError {
                                message: "ZeroDivisionError: integer division by zero".into(),
                                span: expr.1.clone(),
                            });
                        }
                        Ok(Value::Int(a / b))
                    ...
                    B::Modulo => {
                        ...
                        if b == 0 {
                            return Err(RuntimeError {
                                message: "ZeroDivisionError: integer modulo by zero".into(),
                                span: expr.1.clone(),
                            });
                        }
                        Ok(Value::Int(a % b))
                    ...
                    B::Less => Ok(Value::Bool(
                        to_int(&lv, left.1.clone())? < to_int(&rv, right.1.clone())?,
                    )),
                    B::LessEqual => Ok(Value::Bool(
                        to_int(&lv, left.1.clone())? <= to_int(&rv, right.1.clone())?,
                    )),
                    B::Greater => Ok(Value::Bool(
                        to_int(&lv, left.1.clone())? > to_int(&rv, right.1.clone())?,
                    )),
                    B::GreaterEqual => Ok(Value::Bool(
                        to_int(&lv, left.1.clone())? >= to_int(&rv, right.1.clone())?,
                    )),
    ...
    fn call(&mut self, func_name: &str, args: &Vec<ExprS>, call_span: Span) -> RtResult<Value> {
        ...
        match func_name {
            "print" => {
                if args.len() != 1 {
                    return Err(RuntimeError {
                        message: format!(
                            "ArityError: print() takes 1 positional argument but {} given",
                            args.len()
                        ),
            ...
            "input" => {
                if !args.is_empty() {
                    return Err(RuntimeError {
                        message: format!(
                            "ArityError: input() takes 0 positional arguments but {} given",
                            args.len()
                        ),
                        span: call_span.clone(),
                    });
            ...
            "int" => {
                if args.len() != 1 {
                    return Err(RuntimeError {
                        message: format!(
                            "ArityError: int() takes 1 positional argument but {} given",
                            args.len()
                        ),
                        span: call_span.clone(),
                    });
            ...
            "bool" => {
                if args.len() != 1 {
                    return Err(RuntimeError {
                        message: format!(
                            "ArityError: bool() takes 1 positional argument but {} given",
                            args.len()
                        ),
                        span: call_span.clone(),
                    });
        ...
        // user-defined
        let fv = self.env.get(func_name).ok_or_else(|| RuntimeError {
            message: format!("NameError: function '{}' is not defined", func_name),
            span: call_span.clone(),
        })?;
        let func = match fv {
            Value::Function(f) => f,
            _ => {
                return Err(RuntimeError {
                    message: format!("TypeError: '{}' is not callable", func_name),
                    span: call_span.clone(),
                });
            }
        };
        ...
        for (p, v) in func.params.iter().zip(arg_vals.into_iter()) {
            self.env.define(p.clone(), v, call_span.clone())?;
        }
...
fn to_int(v: &Value, span: Span) -> RtResult<i64> {
    match v {
        ...
        _ => Err(RuntimeError {
            message: format!("TypeError: expected Int-compatible, got {:?}", v),
            span,
        }),
    }
}
fn to_bool(v: &Value, span: Span) -> RtResult<bool> {
    match v {
        ...
        Value::Function(_) => Err(RuntimeError {
            message: "TypeError: cannot convert function to bool".into(),
            span,
        }),
    }
}
