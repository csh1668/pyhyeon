...
impl TypeEnv {
    ...
    fn set(&mut self, name: String, ty: Ty) {
        if let Some(cur) = self.frames.last_mut() { cur.insert(name, ty); }
    }
}
...
fn tc_stmt(
    ...
    match &stmt.0 {
            ...
            let new_ty = match (prev, rhs) {
                ...
                (Some(a), b) => {
                    return Err(SemanticError { message: format!(
                        "TypeError: cannot assign value of type {:?} to variable of type {:?}", b, a
                    ), span: value.1.clone() });
                }
            };
        ...
        Stmt::Expr(expr) => { let _ = tc_expr(expr, tenv, ctx)?; Ok(()) }
        Stmt::Return(expr) => {
            ...
            if let Some(ptr) = current_fn_return {
                ...
                unsafe {
                    ...
                    let new = unify_return(old, t).ok_or_else(|| SemanticError {
                        message: format!("TypeError: inconsistent return types in function: {:?} vs {:?}", old, t),
                        span: expr.1.clone(),
                    })?;
                    *ptr = new;
                }
        ...
        Stmt::If { condition, then_block, elif_blocks, else_block } => {
            ...
            with_env(&mut then_env, |e| {
                for s in then_block { let _ = tc_stmt(s, e, ctx, current_fn_return); }
            });
            ...
            for (cond, block) in elif_blocks {
                ...
                with_env(&mut env_i, |e| {
                    for s in block { let _ = tc_stmt(s, e, ctx, current_fn_return); }
                });
            ...
            for (_, a) in &branches { all_assigned.extend(a.iter().cloned()); }
            ...
            for var in all_assigned {
                    ...
                    if assigned_b.contains(&var) {
                        ...
                        if t_b != Ty::Unknown {
                            if let Some(m) = merged {
                                if m != t_b {
                                    return Err(SemanticError { message: format!(
                                        "TypeError: variable '{}' has incompatible types across branches", var
                ...
                if let Some(t) = merged { tenv.set(var, t); }
            }
        ...
        Stmt::While { condition, body } => {
            ...
            with_env(&mut loop_env, |e| {
                for s in body { let _ = tc_stmt(s, e, ctx, current_fn_return); }
            });
            Ok(())
        }
        Stmt::Def { name: _, params, body } => {
            // Type-check function body in its own frame
            tenv.push();
            for p in params { tenv.set(p.clone(), Ty::Unknown); }
            let mut ret: Ty = Ty::Unknown;
            let ret_ptr: *mut Ty = &mut ret;
            for s in body { tc_stmt(s, tenv, ctx, Some(ret_ptr))?; }
            tenv.pop();
...
fn tc_expr(expr: &ExprS, tenv: &mut TypeEnv, ctx: &super::ProgramContext) -> SemanticResult<Ty> {
    match &expr.0 {
        Expr::Literal(lit) => Ok(match lit { crate::parser::ast::Literal::Bool(_) => Ty::Bool, crate::parser::ast::Literal::Int(_) => Ty::Int, crate::parser::ast::Literal::None => Ty::NoneType }),
        ...
        Expr::Binary { op, left, right } => {
            ...
            match op {
                BinaryOp::Add | BinaryOp::Subtract | BinaryOp::Multiply | BinaryOp::FloorDivide | BinaryOp::Modulo =>
                    expect_int_pair(tl, tr, expr.1.clone()).map(|_| Ty::Int),
                BinaryOp::Less | BinaryOp::LessEqual | BinaryOp::Greater | BinaryOp::GreaterEqual =>
                    expect_int_pair(tl, tr, expr.1.clone()).map(|_| Ty::Bool),
                BinaryOp::Equal | BinaryOp::NotEqual => expect_same_or_unknown(tl, tr, expr.1.clone()).map(|_| Ty::Bool),
            ...
            if let Some(bi) = crate::builtins::lookup(func_name) {
                if bi.arity() != args.len() {
                    return Err(SemanticError { message: format!(
                        ...
                        bi.name, bi.arity(), args.len()
                    ), span: expr.1.clone() });
                }
                match bi.name {
                    "print" => { let _ = tc_expr(&args[0], tenv, ctx)?; Ok(Ty::NoneType) }
                    "input" => Ok(Ty::Int),
                    "int" => { let _ = tc_expr(&args[0], tenv, ctx)?; Ok(Ty::Int) }
...
fn ensure_bool(cond: &ExprS, tenv: &mut TypeEnv, ctx: &super::ProgramContext) -> SemanticResult<()> {
    let t = tc_expr(cond, tenv, ctx)?;
    if t != Ty::Bool {
        return Err(SemanticError { message: "TypeError: condition must be Bool".to_string(), span: cond.1.clone() });
    }
    Ok(())
}
...
fn expect_int(t: Ty, span: crate::types::Span) -> SemanticResult<Ty> {
    match t {
        ...
        _ => Err(SemanticError { message: format!("TypeError: expected Int, got {:?}", t), span }),
    }
}
...
fn expect_bool(t: Ty, span: crate::types::Span) -> SemanticResult<Ty> {
    match t {
        ...
        _ => Err(SemanticError { message: format!("TypeError: expected Bool, got {:?}", t), span }),
    }
}
...
fn expect_int_pair(t1: Ty, t2: Ty, span: crate::types::Span) -> SemanticResult<()> {
    match (t1, t2) {
        ...
        _ => Err(SemanticError { message: format!("TypeError: expected Int and Int, got {:?} and {:?}", t1, t2), span }),
    }
}
...
fn expect_bool_pair(t1: Ty, t2: Ty, span: crate::types::Span) -> SemanticResult<()> {
    match (t1, t2) {
        ...
        _ => Err(SemanticError { message: format!("TypeError: expected Bool and Bool, got {:?} and {:?}", t1, t2), span }),
    }
}

fn expect_same_or_unknown(t1: Ty, t2: Ty, span: crate::types::Span) -> SemanticResult<()> {
    if t1 == Ty::Unknown || t2 == Ty::Unknown || t1 == t2 { Ok(()) } else { Err(SemanticError { message: format!(
        "TypeError: equality operands must have same type, got {:?} and {:?}", t1, t2
    ), span }) }
}
...
fn snapshot_env(tenv: &TypeEnv) -> TypeEnv { TypeEnv { frames: tenv.frames.clone() } }
fn with_env<F: FnOnce(&mut TypeEnv)>(tenv: &mut TypeEnv, f: F) { f(tenv); }
fn collect_assigned(base: &TypeEnv, changed: &TypeEnv, out: &mut HashSet<String>) {
    ...
    for (k, v) in changed_top.iter() {
        if base_top.get(k) != Some(v) { out.insert(k.clone()); }
    }
}
fn get_var_type(env: &TypeEnv, var: &str) -> Ty { env.get(var).unwrap_or(Ty::Unknown) }
